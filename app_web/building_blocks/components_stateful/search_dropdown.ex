#......................................................... Purpose ...........................................................
# To show filtered data based on the expression we insert;
#								                .................... Render the component ....................
# Copy this to where you want to use it:
#			<.live_component module={AppWeb.SearchDropdownComponent} id="searchdropdown" />
#                               ..................... Notes & learnings ......................
...
# 3)  We need to insert a "\" so that the line break is escaped in the string that is created in our "<script>";
# 4)  The "@attribs.class" assign is being used to construct and unique "ID" or/and to select and element, so this makes it mandatory;
# 5)  Allow us to insert a new <style> tag in the end of <head> with the specific rules we want to apply to that element;
...
#    use the "send_update" function to send the update and then use these 2 clauses of the "update" function to receive it;
# 8) REMOVE in production!! Used to simulate:
#    a) data coming from an API or database;
#    b) latency in development mode so that we can see the "loading" working properly;
# 9) Hack to display the "loading component" as if we would wrap it inside a normal <%# if @loading do %>COMPONENT <%end%> it
#    would not be rendered at "mount" and then would not render...strange...
# 10) We're using "String.upcase(airport)" to make the flight search case-insensitive;
#                               ...................... Phx events used .......................
...
#   c) search -
#.............................................................................................................................
defmodule AppWeb.SearchDropdownComponent do
  use AppWeb, :live_component

  import AppWeb.StatelessCompLoading                                                                                          #(1)
  ...
  attr :style, :map, default: %{main: "", title: "", form: "", input: "", button: "", flights: "", flight: "", number: "",
                                origin: "", departs: "", arrivals: "", noMatch: "" }
  attr :config, :map, default: %{type: "suggest"}  # options: %{type: "suggest/search"}
  attr :options, :map, default: %{title: "Find a Flight", placeholder: "Airport Code"}
  ...
  def render(assigns) do
    ...
    <div>
      <style>
        .stateful_comp.searchDropdown {
          & input { height: 2.5rem; margin: 1rem 0; border-radius: 0.5rem 0 0 0.5rem; }
          & button { height: 2.5rem; border-radius: 0 0.5rem 0.5rem 0; }
          & .flights { flex: 0 1 50rem; padding: 1rem; margin-top: 1rem; border-radius: 0.5rem; background-color: white; }
          & .flight > div { padding: 0.25rem; }
          & .number { font-size: 2rem; color: magenta; }
          & .origin { color: magenta; }
        }
      </style>
      <div class={"stateful_comp searchDropdown main #{@class}"} style={@style.main}>
        <h1 class="title" style={@style.title}>{@options.title}</h1>
        <form phx-submit="final_search" phx-change={"#{if @config.type == "suggest" do "suggest" else "search" end}"}
              phx-target={@myself} class="width100" style={@style.form}>                                                                      <% #(2)%>
          <input  type="text" name="airport" value={@airport} placeholder={@options.placeholder} autofocus autocomplete="off"
                  readonly={@loading} list={"searchDropdownMatches#{@class}"} phx-debounce="250" style={@style.input}/>
          <button type="submit" style={@style.button}>
            <.svg_search style={%{main: "width: 2.5rem; margin: 0;", inner: ""}}/>
          </button>
        </form>
        <datalist id={"searchDropdownMatches#{@class}"}>
          <option  :for={{code, name} <- @matches} value={code}>{name}</option>
        </datalist>
        <.stateless_comp_loading_arrowBlink style={%{ main: "#{unless @loading do "display: none;" end}", left: "", middle: "",
                                                      right: ""}} />                                                          <% #(9)%>
        <div class="flights" style={@style.flights}>
            <%= for {i, id} <- Enum.with_index(@flights,1) do %>
          <div class={"flight width100 item#{id}"} style={"#{@style.flight}
                                                  #{unless Regex.match?(~r/border-bottom/, @style.flight) do "border-bottom:
                                              #{unless id == length(@flights) do "0.1rem solid lightgrey;" end}" end}"}>
            <div class={"number width50 flex-start number#{id}"} style={"#{@style.number}#{@style["number#{id}"]}"}>Flight #{i.number}</div>
            <div class={"origin width50 flex-end origin#{id}"} style={"#{@style.origin}#{@style["origin#{id}"]}"}>
              {i.origin} to {i.destination}
            </div>
            <div class={"departs width50 flex-start depart#{id}"} style={"#{@style.departs}#{@style["depart#{id}"]}"}>
              Departs: {i.departure_time}
            </div>
            <div class={"arrivals width50 flex-end arrival#{id}"} style={"#{@style.arrivals}#{@style["arrival#{id}"]}"}>
              Arrives: {i.arrival_time}
            </div>
          </div>
            <% end %>
          <div :if={(@flights == []) && (@result)} class="noMatch" style={"#{@style.noMatch}"}>No matches found!</div>
        </div>
      </div>
    </div>
    """
  ...
  @impl true
  def handle_event("final_search", %{"airport" => airport}, socket) do
    send_update(__MODULE__, %{id: "searchdropdown#{socket.assigns.class}", run_final_search: airport})                                               #(6,7)
    socket = assign(socket,
                    airport: airport,
                    flights: [],
                    loading: true,
  ...
  def handle_event("search", %{"airport" => airport}, socket) do
    send_update(__MODULE__, %{id: "searchdropdown#{socket.assigns.class}", run_search: airport})                                                     #(6,7)
    socket = assign(socket,
                    airport: airport,
                    flights: [],
                    loading: true,
  ...
  def handle_event("suggest", %{"airport" => prefix}, socket) do
    matches = suggest(prefix)
    {:noreply, assign(socket, matches: matches)}
  end

  @impl true
  def update(%{run_final_search: airport}, socket) do                                                                         #(7)
      socket =
          assign(socket,
                flights: final_search_by_airport(airport),
                loading: false,
                result: true)
      {:ok, socket}
  end
  ...
  @impl true
  def update(%{run_search: airport}, socket) do                                                                               #(7)
      socket =
          assign(socket,
...
#                                                        Aux Functions                                                        (8a)
                                # Used to present final results
  def final_search_by_airport(airport) do
    Process.sleep(3000)                                                                                                       #(8b)
      list_flights()
      |> Enum.filter(&(&1.origin == String.upcase(airport) || &1.destination == String.upcase(airport)))                      #(10)
  end
  ...
  def list_flights do
    [
      %{
        ...
        origin: "DEN",
        destination: "ORD",
        departure_time: "Jul, 21, 17:00",
        arrival_time: "Jul, 21, 19:30"
      },
      %{
        number: "450",
        origin: "DEN",
